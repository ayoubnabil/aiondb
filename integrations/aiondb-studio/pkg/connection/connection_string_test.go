...
func TestBuildStringFromOptions(t *testing.T) {
	...
	t.Run("oversized pgpass is ignored", func(t *testing.T) {
		...
		assert.NoError(t, err)
		assert.Equal(t, "postgres://username:@localhost:5432/dbname?sslmode=disable", str)
	})

	t.Run("non-regular pgpass is ignored", func(t *testing.T) {
		path := t.TempDir()
	...
	t.Run("invalid url", func(t *testing.T) {
		...
		examples := []string{
			...
			"tcp://blah",
			"foobar",
			"postgres://" + strings.Repeat("x", maxConnectionURLBytes+1),
		}
	...
	t.Run("rebuilt url exceeding size cap is rejected", func(t *testing.T) {
		str, err := BuildStringFromOptions(command.Options{
			Host:   "localhost",
			Port:   5432,
			User:   "username",
			Pass:   strings.Repeat("x", maxConnectionURLBytes+1),
			DbName: "dbname",
		})
...
func TestRedactURL(t *testing.T) {
	assert.Equal(
		t,
		"postgres://user:REDACTED@localhost:5432/db?connect_timeout=30&passfile=REDACTED&sslkey=REDACTED",
		RedactURL("postgres://user:secret@localhost:5432/db?sslkey=/home/user/.ssh/db.key&passfile=/home/user/.pgpass&connect_timeout=30"),
	)
	assert.Equal(t, "postgres://localhost/db", RedactURL("postgres://localhost/db"))
	assert.Equal(t, "[REDACTED]", RedactURL("://bad-url"))
	assert.Equal(t, "[REDACTED]", RedactURL("not a url with password=secret"))
}
...
func TestFormatURL(t *testing.T) {
	examples := []struct {
		...
		{
			name: "remote host containing localhost",
			input: command.Options{
				URL: "postgres://username@localhost.example.com:5432/dbname",
			},
			result: "postgres://username@localhost.example.com:5432/dbname",
		},
	}
