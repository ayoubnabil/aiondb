...
func initClient() {
	...
	if command.Opts.Debug {
		fmt.Println("Opening database connection using string:", connection.RedactURL(cl.ConnectionString))
	}
...
func newAppHTTPServer(handler http.Handler, options command.Options) *http.Server {
	return &http.Server{
...
func startServer() {
	...
	go func() {
		...
		server := newAppHTTPServer(router, options)
