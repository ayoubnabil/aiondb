...
const maxLocalQueryTimeoutSeconds = int64(24 * 60 * 60)
...
func parseMetadata(input string) (*Metadata, error) {
	...
	if fields["timeout"] != "" {
		timeoutSec, err := strconv.ParseInt(fields["timeout"], 10, 64)
		if err != nil {
			return nil, fmt.Errorf(`error initializing "timeout" field: %w`, err)
		}
		if timeoutSec <= 0 {
			return nil, fmt.Errorf(`error initializing "timeout" field: value must be greater than 0`)
		}
		if timeoutSec > maxLocalQueryTimeoutSeconds {
			return nil, fmt.Errorf(`error initializing "timeout" field: value must be less than or equal to %d`, maxLocalQueryTimeoutSeconds)
		}
