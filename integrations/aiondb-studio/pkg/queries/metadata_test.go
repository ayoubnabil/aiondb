...
func Test_parseMetadata(t *testing.T) {
	examples := []struct {
		...
		},
		{
			...
			err:   `error initializing "timeout" field: strconv.ParseInt: parsing "foo": invalid syntax`,
		},
		{
			input: `--pgweb: host="localhost" timeout="0"`,
			err:   `error initializing "timeout" field: value must be greater than 0`,
		},
		{
			input: `--pgweb: host="localhost" timeout="-1"`,
			err:   `error initializing "timeout" field: value must be greater than 0`,
		},
		{
			input: `--pgweb: host="localhost" timeout="9223372036854775807"`,
			err:   `error initializing "timeout" field: value must be less than or equal to 86400`,
		},
		{
