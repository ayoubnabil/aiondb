...
const (
	// Prefix to use for AionDB Studio env vars, ie AIONDB_STUDIO_DATABASE_URL.
	envVarPrefix        = "AIONDB_STUDIO_"
	legacyEnvVarPrefix  = "PGWEB_"
	maxPassfileBytes    = 1024 * 1024
	maxMetricsPathBytes = 1024
	maxURLPrefixBytes   = 256
	maxTCPPort          = 65535

	maxDurationSeconds = uint64((1<<63)-1) / uint64(time.Second)
	maxDurationMinutes = uint64((1<<63)-1) / uint64(time.Minute)
)
...
func ParseOptions(args []string) (Options, error) {
	...
	if err := validateDurationOptions(opts); err != nil {
		return opts, err
	}
	...
	if opts.Passfile == "" {
		...
		if err == nil {
			if stat.Size() > maxPassfileBytes {
				fmt.Printf("[WARN] Pgpass file exceeds maximum size of %d bytes\n", maxPassfileBytes)
			} else if !stat.Mode().IsRegular() {
				fmt.Println("[WARN] Pgpass file is not a regular file")
			} else {
	...
	if opts.Prefix != "" {
		...
		if err := validateURLPrefix(opts.Prefix); err != nil {
			return opts, err
		}
	}
	...
	if (opts.AuthUser == "") != (opts.AuthPass == "") {
		return opts, errors.New("--auth-user and --auth-pass must be set together")
	}
...
func validateDurationOptions(opts Options) error {
	if opts.OpenTimeout < 0 {
		return errors.New("--open-timeout must be greater than or equal to 0")
	}
	if uint64(opts.OpenTimeout) > maxDurationSeconds {
		return fmt.Errorf("--open-timeout must be less than or equal to %d", maxDurationSeconds)
	}
	if uint64(opts.QueryTimeout) > maxDurationSeconds {
		return fmt.Errorf("--query-timeout must be less than or equal to %d", maxDurationSeconds)
	}
	if uint64(opts.RetryDelay) > maxDurationSeconds {
		return fmt.Errorf("--open-retry-delay must be less than or equal to %d", maxDurationSeconds)
	}
	if opts.ConnectionIdleTimeout < 0 {
		return errors.New("--idle-timeout must be greater than or equal to 0")
	}
	if uint64(opts.ConnectionIdleTimeout) > maxDurationMinutes {
		return fmt.Errorf("--idle-timeout must be less than or equal to %d", maxDurationMinutes)
	}
	return nil
}

func validateNetworkOptions(opts Options) error {
	if opts.Port < 1 || opts.Port > maxTCPPort {
		return fmt.Errorf("--port must be between 1 and %d", maxTCPPort)
	}
	if opts.HTTPPort > maxTCPPort {
		return fmt.Errorf("--listen must be less than or equal to %d", maxTCPPort)
	}
	return nil
}
...
func validateURLPrefix(prefix string) error {
	...
	if len(prefix) > maxURLPrefixBytes {
		return fmt.Errorf("--prefix must be less than or equal to %d bytes", maxURLPrefixBytes)
	}
	if strings.ContainsAny(prefix, "?#\\:*") {
		return errors.New("--prefix must not contain query, fragment, wildcard, or backslash characters")
	}
	for _, char := range prefix {
		if char <= ' ' || char == 0x7f {
			return errors.New("--prefix must not contain control or whitespace characters")
		}
	}
	return nil
}

func validateMetricsOptions(opts Options) error {
...
func getPrefixedEnvVar(name string) string {
	...
	if val == "" {
		val = os.Getenv(legacyEnvVarPrefix + name)
	}
	if val == "" {
		val = os.Getenv(name)
		if val != "" {
			fmt.Printf("[DEPRECATION] Usage of %s env var is deprecated, please use %s%s variable instead\n", name, envVarPrefix, name)
		}
	}
	return val
}
