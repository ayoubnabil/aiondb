...
var (
	...
	reRestrictedKeywords        = regexp.MustCompile(`(?i)(^|[^\w])(ALTER|CALL|COPY|CREATE|DELETE|DO|DROP|GRANT|IMPORT|INSERT|LOAD|MERGE|OPEN|REFRESH|REINDEX|REVOKE|TRUNCATE|UPDATE|VACUUM)([^\w]|$)`)
	reRestrictedReadOnlySetting = regexp.MustCompile(`(?i)(^|[^\w])SET\s+((default_transaction_read_only|transaction_read_only)\s*(=|TO)|SESSION\s+CHARACTERISTICS\s+AS\s+TRANSACTION\s+READ\s+WRITE|TRANSACTION\s+READ\s+WRITE)([^\w]|$)`)

	// Comment regular expressions
	reSlashComment = regexp.MustCompile(`(?s)/\*.*?\*/`)
...
func containsRestrictedKeywords(str string) bool {
	...
	return reRestrictedKeywords.MatchString(str) || reRestrictedReadOnlySetting.MatchString(str)
}
