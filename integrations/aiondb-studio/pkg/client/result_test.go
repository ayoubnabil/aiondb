...
func TestCSVEscapesFormulaTextValues(t *testing.T) {
	result := Result{
...
func TestCSVSkipsExtraRowValues(t *testing.T) {
	result := Result{
		Columns: []string{"id"},
		Rows: []Row{
			{1, "unexpected"},
...
func TestResultFormatPadsMissingRowValues(t *testing.T) {
	result := Result{
	...
	assert.NotPanics(t, func() {
		assert.Equal(t, []map[string]interface{}{
			{"col1": "1", "col2": nil, "col3": nil},
		}, result.Format())
	})
}

func TestObjectsFromResultSkipsMalformedRows(t *testing.T) {
	result := Result{
		Rows: []Row{
			{"too-short"},
			{"1", "public", "books", ObjTypeTable},
		},
	}

	assert.NotPanics(t, func() {
		objects := ObjectsFromResult(&result)
		assert.Equal(t, []Object{{OID: "1", Name: "books"}}, objects["public"].Tables)
	})
	assert.Empty(t, ObjectsFromResult(nil))
}
