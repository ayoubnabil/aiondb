...
func TestRedactQueryForLogMasksSQLSecrets(t *testing.T) {
	query := strings.Join([]string{
		"CREATE ROLE app PASSWORD 'secret-password';",
		`ALTER USER app WITH PASSWORD = "secret-password";`,
		"CREATE USER app IDENTIFIED BY plain-secret;",
		"SELECT * FROM settings WHERE token = 'secret-token';",
		"SHOW password_encryption;",
	}, "\n")

	redacted := redactQueryForLog(query)

	assert.NotContains(t, redacted, "secret-password")
	assert.NotContains(t, redacted, "plain-secret")
	assert.NotContains(t, redacted, "secret-token")
	assert.Contains(t, redacted, "PASSWORD [REDACTED]")
	assert.Contains(t, redacted, "SHOW password_encryption;")
}

func TestResultBytesWouldExceed(t *testing.T) {
	assert.False(t, resultBytesWouldExceed(0, Row{"small", int64(42)}, 128))
	assert.True(t, resultBytesWouldExceed(0, Row{strings.Repeat("x", 32)}, 16))
	assert.True(t, resultBytesWouldExceed(121, Row{"small"}, 128))

	assert.False(t, resultRowsBytesWouldExceed([]Row{{"a"}, {"b"}}, 16))
	assert.True(t, resultRowsBytesWouldExceed([]Row{{"abc"}, {"def"}}, 10))
}

func TestAddHistoryRecordCapsStoredHistory(t *testing.T) {
	client := &Client{}
	for i := 0; i < maxHistoryRecords+5; i++ {
		client.addHistoryRecord(fmt.Sprintf("SELECT %d", i))
	}

	assert.Len(t, client.History, maxHistoryRecords)
	assert.Equal(t, "SELECT 5", client.History[0].Query)
}

func TestAddHistoryRecordTruncatesLargeQueries(t *testing.T) {
	client := &Client{}
	client.addHistoryRecord(strings.Repeat("x", maxHistoryQueryBytes+1))

	assert.Len(t, client.History, 1)
	assert.LessOrEqual(t, len(client.History[0].Query), maxHistoryQueryBytes+len("\n-- truncated by pgweb history limit"))
	assert.Contains(t, client.History[0].Query, "truncated by pgweb history limit")
}

func TestHistoryRecordsReturnsCopy(t *testing.T) {
	client := &Client{}
	client.addHistoryRecord("SELECT 1")

	records := client.HistoryRecords()
	records[0].Query = "mutated"

	assert.Equal(t, "SELECT 1", client.HistoryRecords()[0].Query)
	assert.Equal(t, 1, client.HistoryCount())
}

func TestStringsFromFirstColumnRejectsMalformedRows(t *testing.T) {
	values, err := stringsFromFirstColumn(&Result{Rows: []Row{{"public"}, {"private"}}})
	assert.NoError(t, err)
	assert.Equal(t, []string{"public", "private"}, values)

	_, err = stringsFromFirstColumn(&Result{Rows: []Row{{}}})
	assert.EqualError(t, err, "invalid row result")

	_, err = stringsFromFirstColumn(&Result{Rows: []Row{{int64(1)}}})
	assert.EqualError(t, err, "invalid row result")

	values, err = stringsFromFirstColumn(nil)
	assert.NoError(t, err)
	assert.Empty(t, values)
}

func TestEstimatedRowsCountFromResultRejectsMalformedRows(t *testing.T) {
	value, err := estimatedRowsCountFromResult(&Result{Rows: []Row{{float64(42)}}})
	assert.NoError(t, err)
	assert.Equal(t, int64(42), value)

	_, err = estimatedRowsCountFromResult(&Result{Rows: []Row{{}}})
	assert.EqualError(t, err, "invalid estimated row count result")

	_, err = estimatedRowsCountFromResult(&Result{Rows: []Row{{"bad"}}})
	assert.EqualError(t, err, "invalid estimated row count result")
}

func TestServerVersionFromResultRejectsMalformedRows(t *testing.T) {
	version, err := serverVersionFromResult(&Result{Rows: []Row{{"PostgreSQL 14.5"}}})
	assert.NoError(t, err)
	assert.Equal(t, "PostgreSQL 14.5", version)

	_, err = serverVersionFromResult(&Result{Rows: []Row{{}}})
	assert.EqualError(t, err, "invalid server version result")

	_, err = serverVersionFromResult(&Result{Rows: []Row{{int64(1)}}})
	assert.EqualError(t, err, "invalid server version result")
}

func TestNewFromUrlRejectsOversizedConnectionString(t *testing.T) {
	client, err := NewFromUrl("postgres://"+strings.Repeat("x", maxConnectionStringBytes+1), nil)

	assert.Nil(t, client)
	assert.Error(t, err)
	assert.Contains(t, err.Error(), "connection string exceeds maximum size")
}
