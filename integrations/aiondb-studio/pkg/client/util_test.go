...
func TestContainsRestrictedKeywordsBlocksReadonlyBypasses(t *testing.T) {
	examples := []string{
		"SET default_transaction_read_only=off; ALTER TABLE users ADD COLUMN owned text",
		"SET transaction_read_only TO off; UPDATE/**/users SET admin = true",
		"DELETE/**/FROM users WHERE id = 1",
		"ALTER TABLE users ADD COLUMN owned text",
		"DROP/*comment*/TABLE users",
	}

	for _, ex := range examples {
		t.Run(ex, func(t *testing.T) {
			assert.True(t, containsRestrictedKeywords(ex))
		})
	}
}
