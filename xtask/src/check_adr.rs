...
pub fn run(opts: CheckAdrOptions) -> Result<(), String> {
    ...
    for (name, f) in &selected {
        let mut result = f(&root);
        result
            .violations
            .retain(|violation| !allowlist.contains(&format!("{name}:{violation}")));
...
fn load_allowlist(root: &Path) -> std::collections::BTreeSet<String> {
    let Ok(content) = fs::read_to_string(root.join(".check-adr-allow")) else {
