#!/bin/bash
# Fast TCK run using prebuilt debug binary, file by file.
cd "$(dirname "$0")"
base_dir="../testing/cypher-tck/features"
bin="./target/debug/cypher-tck"
...
for feature_file in $(find "$base_dir" -name "*.feature" | sort); do
  ...
  line=$(RUST_MIN_STACK=16777216 timeout 60 "$bin" --file "$fname" 2>&1 | grep "Pass rate")
  if [ -n "$line" ]; then
