...
image. The `Docker` GitHub Actions workflow publishes those `main` and SHA tags
on every push to `main`, so a normal `docker compose up` does not build the
database locally. For production-like tests, set explicit secrets, mount
