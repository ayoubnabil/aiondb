...
struct StorageStateWriteGuard<'a> {
...
impl InMemoryStorage {
    fn ensure_available(&self) -> DbResult<()> {
    ...
    fn log_wal(&self, record: &aiondb_wal::WalRecord) -> DbResult<()> {
        if let Some(wal) = &self.wal {
            wal.log(record)?;
        }
        Ok(())
    ...
    fn log_wal_batch(&self, records: &[aiondb_wal::WalRecord]) -> DbResult<()> {
        if records.is_empty() {
            return Ok(());
        }
        if let Some(wal) = &self.wal {
    ...
    fn log_wal_autocommit(&self, records: &[aiondb_wal::WalRecord]) -> DbResult<Option<Lsn>> {
    ...
    fn log_wal_autocommit_dml_owned(&self, record: aiondb_wal::WalRecord) -> DbResult<Option<Lsn>> {
        self.clear_index_count_caches();
        let Some(wal) = &self.wal else {
            return Ok(None);
        };
    ...
    fn read_state(&self) -> DbResult<PlRwLockReadGuard<'_, StorageState>> {
        self.ensure_available()?;
        Ok(self.state.read())
    }

    fn write_state(&self) -> DbResult<StorageStateWriteGuard<'_>> {
        self.ensure_available()?;
    ...
    fn is_autocommit_txn(txn: TxnId) -> bool {
        txn == TxnId::default()
    }

    fn validate_row_width(descriptor: &TableStorageDescriptor, row: &Row) -> DbResult<()> {
    ...
    fn active_txn_mut(state: &mut StorageState, txn: TxnId) -> DbResult<&mut PendingTransaction> {
        state
            .active_txns
            .get_mut(&txn)
    ...
    fn table_view(state: &StorageState, txn: TxnId, table_id: RelationId) -> Option<TableView<'_>> {
    ...
    fn next_reserved_tuple_id(state: &StorageState, table_id: RelationId) -> DbResult<TupleId> {
    ...
    fn load_base_latest_row_projected(
        &self,
        state: &StorageState,
        table: &TableData,
        table_id: RelationId,
    ...
    fn load_base_visible_row_projected(
        &self,
        state: &StorageState,
        table: &TableData,
        table_id: RelationId,
    ...
    fn load_base_latest_value_matches_any_filter(
        &self,
        state: &StorageState,
        table: &TableData,
        table_id: RelationId,
    ...
    fn load_base_visible_value_matches_any_filter(
        &self,
        state: &StorageState,
        table: &TableData,
        table_id: RelationId,
    ...
    fn hydrate_base_tuple_for_write(
        &self,
    ...
    fn hydrate_paged_persisted_state(&self, mut persisted: StorageState) -> DbResult<StorageState> {
        let Some(paged_tables) = &self.paged_tables else {
            return Ok(persisted);
        };
    ...
    fn clone_hydrated_persisted_state(&self, state: &StorageState) -> DbResult<StorageState> {
        self.hydrate_paged_persisted_state(state.clone())
    }

    fn persist_paged_state(
        &self,
        state: &StorageState,
        durable_lsn: Lsn,
    ...
    fn persist_prepared_paged_state(
        &self,
    ...
    fn log_incremental_paged_full_page_images(
        &self,
        persisted_state: &StorageState,
        changed_tables: Option<&[RelationId]>,
    ) -> DbResult<()> {
    ...
    fn build_full_page_image_batch_records(
        relation_id: RelationId,
    ...
    fn build_page_patch_batch_records(
        relation_id: RelationId,
    ...
    fn build_page_set_u64_batch_records(
        relation_id: RelationId,
    ...
    fn extract_compact_u64_update(segments: &[(u16, Vec<u8>)]) -> Option<(u16, u64)> {
        if segments.len() != 1 {
            return None;
        }
    ...
    fn build_disk_index_page_records(
        disk_index_dir: &Path,
        relation_id: RelationId,
        pages: Vec<(u64, Vec<u8>)>,
    ...
    fn build_paged_table_page_records(
    ...
    fn refresh_paged_state_after_commit(
        &self,
        state: &mut StorageState,
    ...
    fn offload_committed_rows_to_live_paged_store(&self, state: &mut StorageState) {
    ...
    fn maybe_evict_cold_tables(&self, state: &mut StorageState) {
    ...
    fn cleanup_table_from_active_txns(state: &mut StorageState, table_id: RelationId) {
    ...
    fn should_autovacuum_table(table: &TableData) -> bool {
    ...
    fn vacuum_table_with_index_rebuild_guard(
        &self,
        state: &mut StorageState,
        table_id: RelationId,
    ...
    fn maybe_autovacuum_tables(&self, state: &mut StorageState, changed_tables: &[RelationId]) {
    ...
    fn cleanup_index_from_active_txns(state: &mut StorageState, index_id: IndexId) {
        for pending in state.active_txns.values_mut() {
