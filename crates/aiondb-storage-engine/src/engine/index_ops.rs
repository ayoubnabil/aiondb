...
pub(super) fn indexed_column_update_plan(
    ...
    for (index_id, index) in &state.indexes {
        ...
        // INCLUDE columns are stored as bare ColumnIds (no
        // asc/desc/nulls), so we can't reuse the
        // `index_descriptor_value_changed` helper that walks
        // `IndexKeyColumn`s. Inline the check.
        let include_changed = index
            .descriptor
            .include_columns
            .iter()
