...
fn parse_alter_table_rls_action(sql: &str, cursor: usize) -> DbResult<AlterTableRlsAction> {
    ...
    // ALTER TABLE … OWNER TO <role>: AionDB is single-user-tenant in v0.1
    // and does not track per-table ownership beyond the bootstrap role.
    // pg_dump output and ORM migration scripts emit this routinely; treat
    // it as a no-op so those scripts replay cleanly.
    let mut probe = cursor;
    if consume_word_ci(sql, &mut probe, "owner").is_some()
