...
impl Engine {
    ...
    fn execute_portal_statement_core(
        ...
        match statement {
            ...
            | Statement::Merge(_) => self.with_compat_eval_session(session, || {
                ...
                let mut execute = || {
                    let physical_plan = if let Some(fingerprint) = precomputed_plan_fingerprint {
                ...
                if statement_requires_implicit_transaction(&statement) {
                    ...
                    if std::env::var_os("AIONDB_DEBUG_PORTAL_EXEC").is_some() {
                        eprintln!(
                            "DBG_PORTAL txn active={active_txn} include_catalog={include_catalog} include_storage={include_storage}"
                        );
                    }
    ...
    pub(super) fn execute_portal_statement(
        ...
        let result: DbResult<StatementResult> = (|| {
            if std::env::var_os("AIONDB_DEBUG_PORTAL_EXEC").is_some() {
                eprintln!("DBG_PORTAL outer_statement={statement:?}");
            }
            ...
            {
                if std::env::var_os("AIONDB_DEBUG_PORTAL_EXEC").is_some() {
                    eprintln!("DBG_PORTAL statement={statement:?}");
                }
