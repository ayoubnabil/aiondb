...
enum DistributedScalarAggregateKind {
    Count,
    Sum,
    Min,
    Max,
...
#[derive(Clone, Debug)]
enum DistributedAggregateOutputPlan {
    GroupKey {
        source_index: usize,
    },
    Aggregate {
...
struct DistributedAggregatePlanShape {
    source_plan: aiondb_plan::PhysicalPlan,
    source_output_fields: Vec<ResultField>,
    output_plans: Vec<DistributedAggregateOutputPlan>,
}

enum DistributedAggregateMergeState {
    Value(Option<Value>),
...
fn remote_fragment_client_config(
...
fn execute_remote_internal_plan_blocking(
...
fn compute_insert_row_shard_id(
...
fn distributed_plan_leader_nodes(
...
fn classify_distributed_scalar_aggregate_projection(
...
fn build_distributed_aggregate_plan_shape(
    physical_plan: &aiondb_plan::PhysicalPlan,
...
fn resolve_distributed_aggregate_sort_indices(
...
fn apply_distributed_aggregate_order_and_bounds(
    rows: &mut Vec<Row>,
    order_by: &[aiondb_plan::SortExpr],
    sort_indices: &[usize],
    limit: Option<u64>,
...
fn compare_distributed_sort_values(
    left: &Value,
    right: &Value,
    descending: bool,
...
fn literal_limit_offset_u64(expr: &aiondb_plan::TypedExpr, clause: &str) -> DbResult<Option<u64>> {
    match &expr.kind {
...
fn command_rows_affected(result: ExecutionResult, source: &str) -> DbResult<u64> {
    match result {
...
fn remote_sharded_insert_on_conflict_is_shard_local(
...
fn compat_relation_runtime_width(table: &aiondb_catalog::TableDescriptor) -> usize {
...
fn expr_contains_column_equality(
    expr: &aiondb_plan::TypedExpr,
    left_ordinal: usize,
    right_ordinal: usize,
) -> bool {
...
fn column_ref_ordinal(expr: &aiondb_plan::TypedExpr) -> Option<usize> {
    match &expr.kind {
...
fn evaluate_simple_insert_select_assignment(
    expr: &aiondb_plan::TypedExpr,
    row: &Row,
) -> DbResult<Value> {
...
fn ensure_insert_select_assignment_columns_exist(
    expr: &aiondb_plan::TypedExpr,
    row: &Row,
) -> DbResult<()> {
    match &expr.kind {
...
fn merge_distributed_aggregate_output_state(
...
fn finalize_distributed_aggregate_state(state: DistributedAggregateMergeState) -> DbResult<Value> {
    match state {
...
fn finalize_distributed_avg(sum: Option<Value>, count: i64) -> DbResult<Value> {
    if count <= 0 {
...
fn value_to_i64_count(value: &Value) -> DbResult<i64> {
    match value {
...
fn value_to_f64_for_distributed_avg(value: &Value) -> DbResult<f64> {
    match value {
...
fn strip_sharded_aggregate_global_bounds(
    physical_plan: &aiondb_plan::PhysicalPlan,
...
fn merge_distributed_scalar_aggregate_value(
...
fn distributed_agg_add_value(current: Option<Value>, new_val: &Value) -> DbResult<Value> {
    match current {
...
fn deadline_epoch_ms(deadline: Instant) -> u64 {
    let now = Instant::now();
...
fn explain_unsupported_inner_tag(statement: &Statement) -> Option<&'static str> {
    match statement {
...
fn is_unsupported_cypher_feature(err: &DbError) -> bool {
    err.sqlstate() == SqlState::FeatureNotSupported
}

pub(super) fn is_transaction_not_active_in_storage_error(err: &DbError) -> bool {
    err.report()
        .message
        .contains("transaction is not active in storage")
}

pub(super) fn insert_values_storage_autocommit_candidate(statement: &Statement) -> bool {
...
fn statement_needs_explicit_txn_participant_enrollment(statement: &Statement) -> bool {
    !matches!(
        statement,
        Statement::Begin { .. }
            | Statement::Commit { .. }
...
fn statement_is_read_only_safe(statement: &Statement) -> bool {
    match statement {
        Statement::Begin { .. }
        | Statement::Commit { .. }
        | Statement::Rollback { .. }
...
fn update_targets_pg_catalog_virtual_relation(statement: &Statement) -> bool {
...
fn statement_requires_implicit_transaction_for_ddl(statement: &Statement) -> bool {
    matches!(
        statement,
        Statement::Analyze { .. }
            | Statement::Vacuum { .. }
...
// alongside the tag matrix. The guard in the executor imports it from the
// compat crate to keep a single source of truth for compat tag
// classification.
use aiondb_pg_compat::compat_tag_matrix::is_sensitive_compat_tag;

fn statement_command_tag(statement: &Statement) -> &'static str {
    match statement {
...
fn statement_is_planner_pg_object_command(statement: &Statement) -> bool {
    matches!(
        statement,
        Statement::CreateType(_)
            | Statement::AlterType(_)
...
fn action_for_statement(statement: &Statement) -> Action {
...
fn action_for_compat_command_tag(tag: &str) -> Action {
...
fn target_for_statement(_statement: &Statement) -> Option<AccessTarget> {
