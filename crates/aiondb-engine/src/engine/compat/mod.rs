...
fn sql_may_need_typeorm_index_reflection_order_rewrite(sql: &str) -> bool {
    ...
    .is_some()
        && find_ascii_case_insensitive(sql, "\"a\".\"attnum\" = any (\"ix\".\"indkey\")").is_some()
        && find_ascii_case_insensitive(
            sql,
            "left join \"pg_constraint\" \"cnst\" on \"cnst\".\"conname\" = \"i\".\"relname\"",
        )
        .is_some()
        && find_ascii_case_insensitive(sql, "\"cnst\".\"contype\" is null").is_some()
...
pub(super) fn rewrite_typeorm_index_reflection_order(sql: &str) -> Option<String> {
    ...
    Some(format!(
        "{body} ORDER BY array_position(\"ix\".\"indkey\", \"a\".\"attnum\"), \"a\".\"attnum\"{suffix}"
    ))
}
