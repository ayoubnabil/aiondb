...
fn session_is_superuser(
    ...
    static CACHE: OnceLock<PlRwLock<Vec<CachedSuperuserStatus>>> = OnceLock::new();
    ...
    let catalog_revision = catalog_reader.catalog_revision(txn_id)?;
    let catalog_key = std::ptr::from_ref::<dyn CatalogReader>(catalog_reader).cast::<()>() as usize;
    let cache = CACHE.get_or_init(|| PlRwLock::new(Vec::new()));

    {
        ...
        if let Some(entry) = guard.iter().find(|entry| {
            ...
            return Ok(entry.is_superuser);
        }
    }

    let is_superuser = catalog_reader
...

    {
        ...
        guard.push(CachedSuperuserStatus {
            catalog_key,
            catalog_revision,
            role_name,
            is_superuser,
