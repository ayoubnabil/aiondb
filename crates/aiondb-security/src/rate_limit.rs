...
impl FileBackedAuthRateLimiter {
    ...
    fn acquire_state_file_lock(&self) -> DbResult<(fs::File, PathBuf)> {
        ...
        loop {
            match OpenOptions::new()
                ...
                .open(&lock_path)
            {
                Ok(lock_file) => return Ok((lock_file, lock_path)),
                Err(error) if error.kind() == ErrorKind::AlreadyExists => {
                    if Instant::now() >= deadline {
                        return Err(DbError::internal(format!(
                            "timed out waiting for auth lockout state lock {}",
                            lock_path.display()
                        )));
                    }
                ...
                Err(error) => {
                    return Err(DbError::internal(format!(
                        "failed to create auth lockout state lock {}: {error}",
                        lock_path.display()
                    )));
                }
            }
...
impl StateFileLock {
    fn acquire(limiter: &FileBackedAuthRateLimiter) -> DbResult<Self> {
        let (file, path) = limiter.acquire_state_file_lock()?;
