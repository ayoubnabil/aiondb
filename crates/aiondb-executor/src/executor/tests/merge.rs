        ...
        ExecutionResult::Query { rows, .. } => rows
            ...
            .map(|row| {
                let id = match row.values[0] {
                    Value::Int(id) => id,
                    Value::BigInt(id) => i32::try_from(id).expect("id must fit in int"),
                    ref other => panic!("expected integer id, got {other:?}"),
                };
                let val = match row.values[1] {
                    Value::Int(val) => val,
                    Value::BigInt(val) => i32::try_from(val).expect("val must fit in int"),
                    ref other => panic!("expected integer val, got {other:?}"),
                };
                (id, val)
            })
            .collect::<Vec<_>>(),
        ...
        vec![
            ...
            MergeWhenClausePlan {
                ...
                action: MergeActionPlan::Insert {
                    values: vec![
                        TypedExpr::column_ref("source_id", 0, DataType::Int, false),
                        TypedExpr::column_ref("source_val", 1, DataType::Int, false),
                    ],
                },
            },
        ],
