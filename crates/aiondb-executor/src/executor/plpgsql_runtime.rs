...
pub(super) fn try_invoke_plpgsql_trigger(
    ...
    let block = match parse_plpgsql_block_or_fallback(&normalized_body)? {
        Some(block) => block,
        None => return Ok(None),
    };
...
fn parse_plpgsql_block_or_fallback(body: &str) -> DbResult<Option<aiondb_plpgsql::Block>> {
    match aiondb_plpgsql::parse_block(body) {
        Ok(block) => Ok(Some(block)),
        Err(err) if is_plpgsql_program_limit_parse_error(&err) => {
            Err(DbError::program_limit(err.to_string()))
        }
        Err(_) => Ok(None),
    }
}

fn is_plpgsql_program_limit_parse_error(err: &aiondb_plpgsql::ParseError) -> bool {
    let message = err.message.to_ascii_lowercase();
    message.contains("nesting depth") && (message.contains("limit") || message.contains("maximum"))
}
