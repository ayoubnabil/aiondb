...
#[test]
fn like_escape_clause_is_rejected_explicitly() {
    let err = parse_expression("name LIKE 'foo\\_%' ESCAPE '\\\\'").expect_err("expected error");
    assert!(
        err.to_string()
            .contains("LIKE/ILIKE/SIMILAR TO ... ESCAPE is not supported"),
...
#[test]
fn ilike_escape_clause_in_select_is_rejected_explicitly() {
    let err = parse_prepared_statement("SELECT * FROM t WHERE name ILIKE 'a\\_%' ESCAPE '\\\\'")
