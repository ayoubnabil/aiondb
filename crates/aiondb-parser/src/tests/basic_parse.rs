...
#[test]
fn rejects_alter_role_set_statement() {
    let err = parse_prepared_statement("ALTER ROLE app_user SET search_path TO public")
        .expect_err("expected ALTER ROLE ... SET to be rejected");
    assert!(
        err.to_string()
            .contains("ALTER ROLE ... SET/RESET is not supported"),
        "unexpected error: {err}"
    );
}
