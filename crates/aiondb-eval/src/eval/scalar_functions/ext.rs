...
const MAX_ARRAY_FILL_ELEMENTS: usize = 1_000_000;
...
pub(crate) fn eval_array_fill(args: &[Value]) -> DbResult<Value> {
    ...
    let element_count = array_fill_element_count(&dims)?;
    if element_count > MAX_ARRAY_FILL_ELEMENTS {
        return Err(DbError::program_limit(format!(
            "array_fill() element count {element_count} exceeds limit {MAX_ARRAY_FILL_ELEMENTS}"
        )));
    }
...
fn array_fill_element_count(dims: &[usize]) -> DbResult<usize> {
    let mut total = 1usize;
    for dim in dims {
        total = total.checked_mul(*dim).ok_or_else(|| {
            DbError::program_limit("array_fill() dimension product exceeds platform limits")
        })?;
    }
    Ok(total)
}
