...
#[test]
fn non_contiguous_parameters_fill_sparse_slots() {
    // $1 and $3 without $2. Some protocol adapters produce sparse parameter
    // numbering; the planner keeps the missing slot as text up to max($n).
    ...
    let description = planner
        ...
        .expect("describe sparse parameters");
    assert_eq!(
        description.param_types,
        vec![DataType::Int, DataType::Text, DataType::Text]
    );
}
