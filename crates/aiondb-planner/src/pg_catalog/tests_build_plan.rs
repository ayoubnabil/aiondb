...
fn pg_index_lists_indexes_with_properties() {
    ...
    assert_eq!(vals[4], "{1}"); // indkey (column ordinal 1)
}
...
#[test]
fn removed_fake_pg_catalog_tables_are_not_recognized() {
    let table = "pg_tables";
    assert!(
        !is_pg_catalog_table(table),
        "{table} should not be recognized"
    );
    assert!(
        output_fields_for(table).is_none(),
        "{table} should not describe"
    );
    assert!(
        build_plan(&mock_catalog(), txn(), table, None, None, None)
            .expect("ok")
            .is_none(),
        "{table} should not plan"
    );
}
