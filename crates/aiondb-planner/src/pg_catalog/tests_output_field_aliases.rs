...
fn output_fields_for_pg_stat_activity_use_oid_alias_for_usesysid() {
    ...
    let usesysid = fields
        .iter()
        .find(|field| field.name == "usesysid")
        .expect("usesysid field");
    assert_eq!(usesysid.text_type_modifier, Some(TextTypeModifier::Oid));
}
...
fn pg_attribute_lists_columns() {
    ...
    // "users" has 2 columns; system catalog attributes are emitted after user columns.
    assert!(rows.len() >= 2);
