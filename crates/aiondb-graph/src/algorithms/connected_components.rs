...
impl UnionFind {
    ...
    fn new(n: u32) -> Self {
        Self {
            parent: (0..n).collect(),
            rank: vec![0; u32_to_usize(n)],
        }
    }

    /// Find the representative of the set containing `x` with path compression.
    fn find(&mut self, x: u32) -> u32 {
        let idx = u32_to_usize(x);
    ...
    fn union(&mut self, x: u32, y: u32) -> bool {
        ...
        let rx_idx = u32_to_usize(rx);
        let ry_idx = u32_to_usize(ry);
        match self.rank[rx_idx].cmp(&self.rank[ry_idx]) {
            std::cmp::Ordering::Less => self.parent[rx_idx] = ry,
            std::cmp::Ordering::Greater => self.parent[ry_idx] = rx,
            std::cmp::Ordering::Equal => {
                self.parent[ry_idx] = rx;
                self.rank[rx_idx] += 1;
            }
        }
        true
...
/// compression).
pub fn connected_components(graph: &impl GraphView) -> Vec<u32> {
...
///   given node index.
pub fn strongly_connected_components<'a, F>(
