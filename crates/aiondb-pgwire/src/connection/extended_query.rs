...
fn statement_requires_live_describe(statement: &aiondb_parser::Statement) -> bool {
    statement_might_need_wire_metadata(statement)
}
    ...
    W: AsyncWrite + Unpin,
{
    ...
    pub(super) async fn materialize_deferred_portal_if_needed(
        &mut self,
        ...
        if let Some((statement_name, params)) = deferred_bind {
            self.run_engine({
                let session = session.clone();
                let portal_name = portal_name.to_string();
                move |engine| engine.bind(&session, portal_name, statement_name, params)
            })
            .await?;
        }
        Ok(())
    ...
    pub(super) async fn handle_parse(
        ...
        if let Err(e) = validate_parse_param_casts(query, param_oids) {
            self.enter_extended_query_error_state().await?;
            messages::write_error_response(&mut w, &e);
            w.flush(&mut self.writer).await?;
            return Ok(());
            ...
            .await
        {
            Ok(desc) => {
                if let Err(e) = validate_parse_param_oids(param_oids, &desc.param_types) {
                    ...
                    let requires_live_describe = parsed_statement
                        .as_ref()
                        .is_some_and(statement_requires_live_describe);
                    self.statement_wire_state.insert(
                        ...
                        StatementWireState {
                            ...
                            prepared_desc: (!requires_live_describe).then_some(desc),
    ...
    pub(super) async fn handle_bind(
        ...
        let requires_live_describe = self
            .statement_wire_state
            .get(statement_name)
            .and_then(|state| state.parsed_statement.as_ref())
            .is_some_and(statement_requires_live_describe);
        if requires_live_describe
            || self
                .statement_wire_state
                .get(statement_name)
        ...
        {
            if requires_live_describe {
                if let Some(WireStateCleanupHint::ClosePortal(target_portal)) = self
                    .run_engine({
                        let session = session.clone();
                        let statement_name = statement_name.to_string();
                        move |engine| engine.statement_wire_cleanup_hint(&session, &statement_name)
                    })
                    .await
                    .ok()
                    .flatten()
                {
                    self.materialize_deferred_portal_if_needed(&session, &target_portal)
                        .await?;
                }
            }
            match self
        ...
        let deferred_describe_response = {
            if let Some(cached) = (!requires_live_describe)
                .then(|| {
                    self.statement_wire_state
                        .get(statement_name)
                        .and_then(|state| {
                            state.deferred_describe_response_cache.get(result_formats)
                        })
                        .cloned()
                })
                .flatten()
    ...
    pub(super) async fn handle_describe(
        ...
        match target {
            DescribeTarget::Statement => {
                let cached_desc = self.statement_wire_state.get(name).and_then(|state| {
                    if state
                        .parsed_statement
                        .as_ref()
                        .is_some_and(statement_requires_live_describe)
    ...
    pub(super) async fn handle_execute(
        ...
        let statement_name_for_wire = self
            .portal_wire_state
            .get(portal_name)
            .map(|portal| portal.statement_name.clone());
        let (statement_kind, statement_sql_for_wire, statement_for_wire) = statement_name_for_wire
            .as_ref()
            .and_then(|statement_name| {
                self.statement_wire_state.get(statement_name).map(|state| {
                    (
                        state.parsed_statement_kind,
                        state.query.clone(),
                        state.parsed_statement.clone(),
                    )
                })
            })
            .unwrap_or((ParsedStatementKind::Other, String::new(), None));
        let sql_keyword_needs_wire_metadata = first_sql_keyword(&statement_sql_for_wire)
            .is_some_and(|keyword| {
                matches!(
                    keyword.to_ascii_lowercase().as_str(),
                    "close" | "deallocate" | "execute" | "fetch" | "move" | "show" | "set"
                )
            });
        let needs_wire_metadata = !matches!(statement_kind, ParsedStatementKind::Other)
            ...
            || sql_keyword_needs_wire_metadata;
        ...
        let cleanup_hint_for_execute = if let Some(hint) = wire_metadata
            .as_ref()
            .and_then(|metadata| metadata.cleanup_hint.clone())
        {
            Some(hint)
        } else if let Some(portal_name) = simple_close_portal_target(&statement_sql_for_wire) {
            Some(WireStateCleanupHint::ClosePortal(portal_name))
        } else if needs_wire_metadata {
            if let Some(statement_name) = statement_name_for_wire.as_ref() {
                self.run_engine({
                    let session = session.clone();
                    let statement_name = statement_name.clone();
                    move |engine| engine.statement_wire_cleanup_hint(&session, &statement_name)
        ...
        if let Some(WireStateCleanupHint::ClosePortal(target_portal)) =
            cleanup_hint_for_execute.as_ref()
        {
            self.materialize_deferred_portal_if_needed(&session, target_portal)
                .await?;
        }
        ...
        match engine_result {
            Ok((mut batch, notices)) => {
                ...
                if let Some(hint) = cleanup_hint_for_execute.as_ref() {
...
fn validate_parse_param_casts(query: &str, param_oids: &[u32]) -> Result<(), DbError> {
    for (index, oid) in param_oids.iter().copied().enumerate() {
        if oid == 0 {
            continue;
        }
        let Some(provided_type) = pg_oid_to_data_type(oid) else {
            continue;
        };
        let param_index = index + 1;
        let Some(cast_type) = explicit_dollar_param_cast_type(query, param_index) else {
            continue;
        };
        if !parse_oid_type_matches_cast(&provided_type, cast_type) {
            return Err(DbError::protocol(format!(
                "could not infer consistent type for parameter ${param_index}"
            )));
        }
    }
    Ok(())
...
fn explicit_dollar_param_cast_type(query: &str, param_index: usize) -> Option<&str> {
    let needle = format!("${param_index}::");
...
fn parse_oid_type_matches_cast(provided_type: &DataType, cast_type: &str) -> bool {
    let cast_type = cast_type.to_ascii_lowercase();
    match provided_type {
        DataType::Text => matches!(cast_type.as_str(), "text"),
        DataType::Int => matches!(
            cast_type.as_str(),
            "int" | "int4" | "integer" | "int2" | "smallint"
        ),
        DataType::BigInt => matches!(cast_type.as_str(), "int8" | "bigint"),
        DataType::Boolean => matches!(cast_type.as_str(), "bool" | "boolean"),
        _ => true,
    }
}
