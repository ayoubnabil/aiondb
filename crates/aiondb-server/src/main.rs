...
use aiondb_engine::{
    ...
    Engine, EngineBuilder, QueryEngine, SecretString, SqlState, StartupParams, TransportInfo,
    TransportKind,
};
...
fn run_offline_backup_command(
    ...
    let credential = match (bootstrap_user, bootstrap_password) {
        (Some(user), Some(password)) => {
            match engine.bootstrap_role(&user, &password, true) {
                Ok(()) => {}
                Err(err) if err.sqlstate() == SqlState::UniqueViolation => {}
                Err(err) => return Err(format!("failed to bootstrap offline role: {err}")),
            }
            aiondb_engine::Credential::CleartextPassword {
                user,
                password: SecretString::new(password),
            }
        }
        _ => aiondb_engine::Credential::Anonymous {
            user: "alice".to_owned(),
        },
    };
    ...
    if matches!(
        ...
        match cli.command {
            ...
            CliCommand::Dump => {
                ...
                let bootstrap_user = cli
                    .bootstrap_user
                    .clone()
                    .or_else(|| std::env::var("AIONDB_BOOTSTRAP_USER").ok());
                let bootstrap_password = cli
                    .bootstrap_password
                    .clone()
                    .or_else(|| std::env::var("AIONDB_BOOTSTRAP_PASSWORD").ok());
                match run_offline_backup_command(
                    data_dir.clone(),
                    config,
                    storage_backend,
                    sql,
