...
#[test]
fn bolt_decode_string_rejects_oversized_declared_len_without_allocating() {
    let len = u32::try_from(BOLT_MAX_STRING_BYTES + 1).expect("test length fits u32");
    let bytes = [vec![0xD2], len.to_be_bytes().to_vec()].concat();
    let mut input = bytes.as_slice();

    let err = decode_string(&mut input).expect_err("oversized string must be rejected");

    assert_eq!(err.kind(), io::ErrorKind::InvalidData);
}

#[test]
fn bolt_decode_string_list_rejects_huge_declared_len_without_allocating() {
    let len = u32::try_from(BOLT_MAX_COLLECTION_ITEMS + 1).expect("test length fits u32");
    let bytes = [vec![0xD6], len.to_be_bytes().to_vec()].concat();
    let mut input = bytes.as_slice();

    let err = decode_string_list(&mut input).expect_err("oversized list must be rejected");

    assert_eq!(err.kind(), io::ErrorKind::InvalidData);
}

#[test]
fn bolt_decode_json_map_rejects_huge_declared_len_without_allocating() {
    let len = u32::try_from(BOLT_MAX_PARAMETER_ENTRIES + 1).expect("test length fits u32");
    let bytes = [vec![0xDA], len.to_be_bytes().to_vec()].concat();
    let mut input = bytes.as_slice();

    let err = decode_string_keyed_json_map(&mut input).expect_err("oversized map must be rejected");

    assert_eq!(err.kind(), io::ErrorKind::InvalidData);
}

#[test]
fn bolt_auth_metadata_rejects_oversized_principal_without_allocating() {
    let principal_len =
        u8::try_from(BOLT_MAX_AUTH_PRINCIPAL_BYTES + 1).expect("test length fits u8");
    let mut payload = vec![0xB1, BOLT_HELLO_SIGNATURE, 0xA1, 0x89];
    payload.extend_from_slice(b"principal");
    payload.extend_from_slice(&[0xD0, principal_len]);

    let err =
        decode_hello_metadata(&payload).expect_err("oversized principal must be rejected early");
            ...
            "statuses" => {
                ...
                for _ in 0..status_len {
                    ...
                    match status_key.as_str() {
                        ...
                        "status_description" => {
                            status_description =
                                Some(decode_string(&mut remaining).expect("status_description"));
                        }
                        "title" => {
...
fn assert_failure_message(
    ...
    for _ in 0..map_len {
        ...
        match key.as_str() {
            ...
            "neo4j_code" => neo4j_code = Some(decode_string(&mut remaining).expect("neo4j_code")),
            "message" => message = Some(decode_string(&mut remaining).expect("message")),
            "gql_status" => gql_status = Some(decode_string(&mut remaining).expect("gql_status")),
            "description" => {
                description = Some(decode_string(&mut remaining).expect("description"))
            }
            "diagnostic_record" => {
...
async fn chunked_message_roundtrip() {
    ...
    let server = tokio::spawn(async move {
        ...
        let payload = read_chunked_message(&mut stream)
            .await
            .expect("read payload");
        assert_eq!(payload, b"hello bolt");
        write_chunked_message(&mut stream, b"ack")
            .await
            .expect("write ack");
    });
...
async fn bolt_run_pull_returns_query_records() {
    ...
    assert_eq!(
        record,
        encode_record_message(&[aiondb_engine::Value::Int(1)]).unwrap()
    );
...
async fn bolt_run_write_mode_returns_update_summary_and_persists_changes() {
    ...
    assert_eq!(
        record,
        encode_record_message(&[aiondb_engine::Value::Int(7)]).unwrap()
    );
...
async fn bolt_pull_n_pages_records() {
    ...
    assert_eq!(
        record2,
        encode_record_message(&[aiondb_engine::Value::Int(2)]).unwrap()
    );
...
async fn bolt_route_rejects_invalid_bookmarks_field() {
    ...
    assert_eq!(
        response,
        encode_failure_message(BOLT_REQUEST_FAILURE_CODE, "bolt value must be a list",)
    );
...
async fn bolt_begin_run_commit_returns_record() {
    ...
    assert_eq!(
        record,
        encode_record_message(&[aiondb_engine::Value::Int(3)]).unwrap()
    );
    let _ = read_chunked_message(&mut client)
        .await
        .expect("read summary");
...
async fn bolt_run_rejects_non_string_database_metadata() {
    ...
    assert_eq!(
        ...
        encode_failure_message(
            BOLT_REQUEST_FAILURE_CODE,
            "Bolt db metadata must be a string"
        )
    );
...
async fn bolt_run_returns_graph_metadata_compat_calls() {
    ...
    async fn run_and_collect_single_column(
        ...
        loop {
            let message = read_chunked_message(client).await.expect("read response");
...
fn encode_record_message_supports_arrays() {
    ...
    assert_eq!(
        encoded,
        vec![0xB1, BOLT_RECORD_SIGNATURE, 0x91, 0x92, 0x01, 0x81, b'x']
    );
}
...
fn encode_record_message_supports_bolt_date() {
    let date = time::Date::from_calendar_date(1970, time::Month::January, 2).expect("date");
    let encoded = encode_record_message(&[aiondb_engine::Value::Date(date)]).expect("encode date");
    assert_eq!(
        encoded,
        vec![0xB1, BOLT_RECORD_SIGNATURE, 0x91, 0xB1, b'D', 0x01]
    );
}
...
fn encode_record_message_supports_bolt_time_with_offset() {
    ...
    let encoded = encode_record_message(&[aiondb_engine::Value::TimeTz(time, offset)])
        .expect("encode timetz");
